...
class ItemsViewModel extends ChangeNotifier {
  ...
  Future<void> _loadItems() async {
    _isLoading = true;
    _error = null;
    // Don't notifyListeners yet, wait until loading is complete
    ...
    try {
      ...
      if (itemsJson != null) {
       ...
       _error = e.toString(); // Set error even for fallback failure
    } finally {
      _isLoading = false;
      notifyListeners(); // Notify listeners only after loading (success or failure)
    }
  }
  ...
  Future<void> addItem(Item item) async {
    ...
    try {
      ...
      _updateFavorites();
      _error = null;
       print('ItemsViewModel: Added item. Total items: ${_items.length}'); // Debug print
    } catch (e) {
      // Fallback to local storage if API fails
      _items.add(item);
      await _saveItems();
      _updateFavorites();
       _error = e.toString(); // Set error even for fallback failure
        print('ItemsViewModel: Added item (fallback). Total items: ${_items.length}'); // Debug print
    } finally {
      _isLoading = false;
      notifyListeners();
    }
