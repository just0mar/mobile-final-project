// This is a basic Flutter widget test.
//
// To perform an interaction with a widget in your test, use the WidgetTester
// utility in the flutter_test package. For example, you can send tap and scroll
// gestures. You can also use WidgetTester to find child widgets in the widget
// tree, read text, and verify that the values of widget properties are correct.

import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:provider/provider.dart';
import 'package:myapp/test_utils.dart';
import 'package:myapp/viewmodels/auth_viewmodel.dart';
import 'package:myapp/viewmodels/items_viewmodel.dart';

void main() {
  testWidgets('App smoke test', (WidgetTester tester) async {
    // Build our app and trigger a frame
    await tester.pumpWidget(const TestMyApp());
    
    // Wait for SharedPreferences to load
    await tester.pumpAndSettle();

    // Verify that our app is built correctly
    expect(find.byType(MaterialApp), findsOneWidget);
    
    // Verify that providers are available
    expect(find.byType(AuthViewModel), findsOneWidget);
    expect(find.byType(ItemsViewModel), findsOneWidget);
  });
}
